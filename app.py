...
# CSS PARA NITIDEZ E DESTAQUE
st.markdown("""
    <style>
    [data-testid="stAppViewContainer"], .stApp { background-color: #000000 !important; }
    .stTooltipIcon { filter: invert(1) brightness(2); transform: scale(1.3); }
...
# LISTA INTEGRAL (DISTRITOS + MUNICÍPIOS RMSP)
lista_geo = sorted([
...
with st.form("main_calc"):
    ...
    with c2: vida = st.number_input("🏠 CUSTO VIDA:", min_value=0.0, help="Preenchimento Opcional: Moradia, alimentação e contas fixas para apurar o rendimento real após custos de sobrevivência.")
...
if btn and sal > 0:
    ...
    # LÓGICA DE TEXTO PARA DESLOCAMENTO INTERNO OU EXTERNO
    local_txt = f"por dentro de <b>{moradia}</b>" if moradia == trabalho else f"entre <b>{moradia}</b> e <b>{trabalho}</b>"
    
    # SÍNTESE FINAL REFINADA
    sintese_finan = f"Ao custo do transporte (R$ {custo_t:,.2f}), soma-se o custo de vida (R$ {vida:,.2f}), resultando em um confisco financeiro severo." if vida > 0 else f"O custo direto do seu transporte consome R$ {custo_t:,.2f} do seu rendimento mensal."
    
    st.markdown(f"""
        <div class="sintese-box">
            <b>SÍNTESE DO IMPACTO:</b><br>
            Ao se deslocar {local_txt}, você dedica <span style="color:#FFCC00">{h_total:.0f} horas</span> mensais que não são pagas por ninguém. 
            {sintese_finan} Na prática, sua hora real de trabalho vale apenas <b>R$ {max(0, v_hora_real):.2f}</b>. 
            Isso demonstra como a mobilidade urbana na RMSP expropria tanto sua renda quanto seu tempo de vida.
        </div>
    """, unsafe_allow_html=True)
