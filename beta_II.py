...
st.markdown("""
    ...
    .stTooltipIcon {
        color: #E63946 !important; /* Vermelho para destacar */
...
with st.form("form_beta_ii_v_final_ajustada"):
    ...
    with r2: custo_vida = st.number_input("CUSTO DE VIDA FIXO (R$):", min_value=0.0, step=50.0, value=0.0, 
                                         help="OPCIONAL. Gastos de sobrevivência (Aluguel, Comida, Contas). Este valor NÃO entra no cálculo do Confisco, ele apenas define sua sobra final após todas as despesas.")
    
    st.markdown('<h4 style="color:#FFCC00; font-size:0.9rem;">🚌 CUSTOS NO TRECHO (DIÁRIO IDA+VOLTA)</h4>', unsafe_allow_html=True)
...
if submit:
    ...
    # CONFISCO NOMINAL: TARIFA TOTAL + VALOR DO TEMPO EXPROPRIADO (Trabalho não pago)
