...
with st.form("form_beta_ii_modais_completos"):
    ...
    with r2: custo_vida = st.number_input("CUSTO DE VIDA FIXO (R$):", min_value=0.0, step=50.0, value=0.0, 
                                         help="OPCIONAL. Gastos básicos. Se zero, o sistema foca apenas na perda pelo transporte.")
    ...
    g4, g5 = st.columns(2)
    with g4: p_app = st.number_input("📱 APP/TÁXI (R$):", min_value=0.0, value=0.0)
    with g5: p_car = st.number_input("🚗 CARRO/COMBUSTÍVEL (R$):", min_value=0.0, value=0.0)
...
if submit:
    ...
    # CONFISCO NOMINAL: TARIFA TOTAL + VALOR DO TEMPO (TRABALHO NÃO PAGO)
    valor_tempo_expro = h_trecho_mensal * v_hora_nom
    confisco_total = custo_transp_mensal + valor_tempo_expro
    
    # RENDIMENTO REAL (O quanto a hora vale após transporte e tempo de trecho)
    ...
    st.markdown(f"""
        <div style="background: #111; padding: 25px; border: 1px solid #E63946; text-align: center;">
            <div style="display: flex; justify-content: space-around; align-items: center;">
                ...
                <div style="color: #FFCC00; font-size: 1.1rem; font-family: 'Arial Black';">💼 {workplace if 'workplace' in locals() else trabalho}</div>
            </div>
