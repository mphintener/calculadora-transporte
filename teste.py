...
st.markdown("""
    ...
    /* Ajuste para evitar que faixas cubram o título */
    .main-title { margin-top: -30px; padding-bottom: 20px; }
...
# TÍTULO COM CLASSE CSS PARA EVITAR SOBREPOSIÇÃO
st.markdown('<h1 class="main-title">📊 CALCULADORA DO TRECHO</h1>', unsafe_allow_html=True)
...
m1, m2 = st.columns(2)
mun_moradia = m1.selectbox("MUNICÍPIO (Moradia)", municipios_rmsp, index=None, placeholder="Selecione o Município")
if mun_moradia == "São Paulo":
    dist_moradia = m2.selectbox("DISTRITO (Moradia)", distritos_sp, index=None, placeholder="Selecione o Distrito")
else:
    dist_moradia = m2.text_input("BAIRRO/DISTRITO (Moradia)", placeholder="Digite seu bairro")

st.markdown("### 🏢 LOCAL DE TRABALHO")
t1, t2, t3 = st.columns(3)
mun_trabalho = t1.selectbox("MUNICÍPIO (Trabalho)", municipios_rmsp, index=None, placeholder="Selecione o Município")
if mun_trabalho == "São Paulo":
    dist_trabalho = t2.selectbox("DISTRITO (Trabalho)", distritos_sp, index=None, placeholder="Selecione o Distrito")
else:
...
if st.button("EFETUAR DIAGNÓSTICO"):
    if not mun_moradia or not mun_trabalho:
        st.error("Por favor, selecione os municípios de moradia e trabalho.")
    else:
